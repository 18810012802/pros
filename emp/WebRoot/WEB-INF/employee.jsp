...
ArrayList<UserForm> al=(ArrayList)request.getAttribute("al");
...
<html>
 ...
 <tr>
    ...
    <td><form action="/emp/updateUser.do?type=addInfo" method="post"><div align="center">
   <table class="abc" width="100%"  border="1" bordercolor="#551A8B" cellpadding="0" cellspacing="0">
        
    <tr>
    <td colspan="2" align="center"><img src="images/gyxx.png" width="149" height="63" /></td>
    </tr>
   <tr>
 ...
 <option selected>楼号
    ...
    <tr> 
     <td width="25%" align="right">姓名：</td>
       <td width="25%"><input  name="ename" type="text" size="15" value=""/></td>
    
       <td width="25%" align="right">员工号：</td>
       <td width="25%"><input name="userno" type="text" size="15" value="" /></td>
     ...
     <tr> 
      <td width="25%" align="right">性别：</td>
       <td width="25%"><input name="sex" type="text" size="15" value="" /></td>
        ...
        <td width="25%" align="right">入职时间：</td>
       <td width="25%"><input  name="intime" type="text" size="15" value=""/></td>
            </tr> 
      <tr> 
      
        <td width="25%" align="right">年龄：</td>
       <td width="25%"><input  name="age" type="text" size="15" value=""/></td>
        <td width="25%" align="right">电话：</td>
       <td width="25%"><input name="phone" type="text" size="15" value="" /></td>
     </tr> 
      <tr> 
      
        <td width="25%" align="right">电子邮件：</td>
       <td width="25%"><input  name="email" type="text" size="15" value=""/></td>
        <td width="25%" align="right">地址：</td>
       <td width="25%"><input name="address" type="text" size="15" value="" /></td>
     </tr> 
       <tr> 
        <td width="25%" align="right">密码：</td>
       <td width="25%"><input  name="passsword" type="text" size="15" value=""/></td>
       <td width="25%" align="right">职位：</td>
       <td width="25%"><select name="job" ><option value="manager">管理员<option value="cleaner">普通员工</select></td>
  ...
  <tr align="center">
   ...
   <th>删除</th>
		 ...
		 {
		  UserForm ib=(UserForm)al.get(i);
		 %>
		 <tr align="center">
		 
		 <td><%=ib.getYno() %></td> <td><%=ib.getBno() %></td>  <td><%=ib.getUserno() %><td><%=ib.getEname() %></td> <td><%=ib.getSex() %></td> <td><%=ib.getAge() %></td> <td><%=ib.getJob() %></td>
		   <td><%=ib.getEmail()%></td></td><td><%=ib.getAddress()%> </td><td><%=ib.getPhone()%></td></td><td><%=ib.getIntime()%> </td>
		  <td><a href="/emp/updateUser.do?type=updateInfo&userno=<%=ib.getUserno()%>">修改</a></td>
		  <td><a href="/emp/updateUser.do?type=deleteInfo&userno=<%=ib.getUserno()%>">删除</a></td> 
		 </tr>
		 <%
		 }
		  ...
		  %>
		   <tr>
             <td align="center" valign="middle" height="10" colspan="13"  bgcolor="#F0F0F0"><form action="/emp/updateUser.do?type=selectByUserno" method="post">请输入员工号：<input type="text" size="15" name="userno"/><input width="25"  type="image" src="images/cx.JPG"/></form></td> 
          </tr>
    ...
    {
    %>
    <a href="/emp/updateUser.do?type=left&pageNow=<%=i %>">[<%=i%>]</a>
    <%
    }
     %>
     </td>
